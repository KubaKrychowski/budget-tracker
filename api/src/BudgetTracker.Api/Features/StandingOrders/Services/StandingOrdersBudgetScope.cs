...
/// <summary>
/// Wybór budżetu ekranu zleceń stałych — ta sama reguła (<see cref="BudgetScope"/>) co na pozostałych ekranach,
/// żeby domyślnie wszystkie pokazywały ten sam budżet.
/// </summary>
public sealed class StandingOrdersBudgetScope(AppDbContext db, TimeProvider clock)
{
    ...
    public async Task<IReadOnlyList<StandingOrdersBudgetOptionResponseDto>> OptionsAsync(CancellationToken ct) =>
        await db.Budgets
            .OrderByDescending(b => b.Month)
            .ThenByDescending(b => b.Id)
            .Select(b => new StandingOrdersBudgetOptionResponseDto(b.BusinessId, b.Name, b.Month, b.DisabledAt != null))
            .ToListAsync(ct);
    ...
    public static Guid? Resolve(
        IReadOnlyList<StandingOrdersBudgetOptionResponseDto> budgets, Guid? requested, DateOnly referenceDate)
    {
